...
EMPRESA_CEO_PROVIDER_FALLBACK (`CeoService.callCeo`, `agentId="ceo"` siempre): se publica cada vez que una llamada del CEO cae del proveedor configurado (`llm.ceo-provider`) a `OllamaLlmProvider` — ver `docs/superpowers/specs/2026-09-18-nvidia-ceo-provider-design.md`. `data: {operation, reason}` (`reason` uno de `RATE_LIMIT_OR_ERROR|BUDGET_EXHAUSTED`): `RATE_LIMIT_OR_ERROR` cuando el proveedor configurado lanzó una excepción o devolvió una respuesta vacía/truncada; `BUDGET_EXHAUSTED` cuando el tope diario de llamadas a NVIDIA (`AiBudgetService`) ya estaba agotado antes de intentar la llamada. Nunca se publica cuando el proveedor configurado ya es Ollama (no hay a dónde caer).
