...
**No verificado en vivo todavía**: a diferencia de otras rondas de este archivo, esta feature **no** se probó contra un Neo4j/Docker real — no hay evidencia en este worktree de que alguien haya corrido `ensureEngineeringTeam()` contra una instancia real y confirmado los 9 agentes, los 5 `MEMBER_OF`, la relación `LEADS` de Neo, o el backfill idempotente de `Agent.model`. Coherente con el criterio ya acordado para toda la familia `*MemoryService` (sin test unitario directo, verificación en vivo en su lugar), pero esa verificación en vivo queda pendiente como hueco conocido — no se afirma acá que haya ocurrido. Sí está cubierto por tests unitarios reales (formatters del chat, wiring de resolución de modelo en `AgentRuntimeTest`, el nuevo `CompanyControllerTest`).
