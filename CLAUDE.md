...
- `EvidenceAcquisitionService.searchEvidence(query)` → `List<EvidenceCandidate>` (usa `evidence.web-search.country`/`.language` como default por si el llamador no los especifica). `confirmReachable(candidate)` → `AgentResult.Evidence` con **`verified=false` siempre** — confirma que la URL respondió con contenido, no que el contenido respalda el `claim`. Esto es una desviación deliberada del código de referencia del doc (que en su §11 marcaba `verified=true` solo por un fetch exitoso); hacerlo así habría contradicho todo lo que `AgentResultValidator`/`EvidenceValidationGate`/`ContradictionDetector` ya hacen en este proyecto para que un `verified=true` signifique algo real. La capa de verificación semántica (¿el contenido realmente respalda la afirmación?) queda pendiente, tal como el propio documento reconoce.
