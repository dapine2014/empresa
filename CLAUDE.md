...
Cubierto por `EvidenceBindingGateTest` (8 casos aislados, sin Neo4j/Ollama) y por 2 casos nuevos en `AgentRuntimeTest` que simulan el flujo completo (reintento cuando no cita, agotamiento cuando nunca cita). Verificado en vivo: misión real de 5 agentes con `qwen3:8b` — los 5 citaron su evidencia confirmada en el primer intento (`validation passed attempt=1` para los 5, cero rechazos de binding) — confirma que el gate no genera falsos positivos contra tráfico real, aunque no se forzó un caso real de "buscó y no citó" en esa corrida (con `qwen3:8b` es infrecuente, ver "Modelo de agente" más abajo); el camino de rechazo+reintento sí está probado end-to-end contra el mismo contrato de datos real (`AgentTaskOutcome`) vía el test mockeado de `AgentRuntimeTest`.
