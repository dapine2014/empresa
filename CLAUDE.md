...
Si **cualquier** tarea de agente falla — tras agotar sus reintentos internos, ver "Reintento de resultados de agente" más abajo — `allOf().join()` propaga la excepción y la misión entera termina en `FAILED` (no hay resultado parcial ni reintento a nivel de misión).
...
**Pendiente, no bloqueante**: scoring de evidencia, y **Evidence Binding** — un gate explícito que detecte "el agente buscó pero no usó los resultados" y lo trate como resultado inválido (retroalimentando el mismo bucle de reintento que ya existe), en vez de depender de que el modelo lo haga espontáneamente. Con `qwen3:8b` este problema se redujo mucho (5/5 en la última corrida) pero no hay garantía de que sea 100% consistente en todas las corridas — el gate seguiría siendo la protección determinista correcta a construir después. La verificación semántica léxica (`ClaimRelevanceChecker`) ya está implementada y conectada (ver arriba); lo que sigue pendiente ahí es la extracción/NLP real del dato concreto, no solo del tema.
